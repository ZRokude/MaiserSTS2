...
public abstract class AmuletPowerBase : CustomPowerModel
{

    public virtual async Task OnAmuletRemoved(Creature creature, int powerCount, ICombatState combatState)
    {
    ...
    {
        if (creature.Player.HasPower<MaiserNeighborhoodHeroPower>())
        {
    ...
    {
        if (creature.Player.HasPower<VincentThePeacekeeperPower>())
        { 
    ...
    private async Task GetVincentSubtractCostEffect(Player owner, int times)
    {
        ...
        if (cards.Any(c => c is VincentThePeacekeeper))
        {
